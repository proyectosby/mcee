...
use Yii;
use app\models\PlantaDocenteAdministrativa;
...
/**
 * PlantaDocenteAdministrativaController implements the CRUD actions for PlantaDocenteAdministrativa model.
 */
class PlantaDocenteAdministrativaController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    /**
     * Lists all PlantaDocenteAdministrativa models.
     * @return mixed
     */
    public function actionIndex($guardado = 0)
    {
        $dataProvider = new ActiveDataProvider([
            'query' => PlantaDocenteAdministrativa::find(),
        ]);
    ...
    /**
     * Displays a single PlantaDocenteAdministrativa model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new PlantaDocenteAdministrativa model.
    ...
    public function actionCreate()
    {
		...
		if( Yii::$app->request->post('PlantaDocenteAdministrativa') )
			$data = Yii::$app->request->post('PlantaDocenteAdministrativa');
		...
		for( $i = 0; $i < $count; $i++ )
		{
			$models[] = new PlantaDocenteAdministrativa();
		}
							
        $dataTiposDocumento  = TiposDocumentos::find()->where( 'estado=1' )->andWhere( "categoria='Planta'")->all();
		$tiposDocumento 	 = ArrayHelper::map( $dataTiposDocumento, 'id', 'descripcion' );
		
		$dataEstados  = Estados::find()->where( 'id=1' )->all();
        $estados 	  = ArrayHelper::map( $dataEstados, 'id', 'descripcion' );

        if (PlantaDocenteAdministrativa::loadMultiple($models, Yii::$app->request->post() )) {			
			...
			foreach( $models as $key => $model) {
				...
				if( $file ){
					...
					$carpeta = "../documentos/plantaDocenteAdministrativa/";
					if (!file_exists($carpeta)) {
						mkdir($carpeta, 0777, true);
					}
					
					//Construyo la ruta completa del archivo a guardar
					$rutaFisicaDirectoriaUploads  = "../documentos/plantaDocenteAdministrativa/";
			...
			if (!PlantaDocenteAdministrativa::validateMultiple($models)) {
        ...
        $model = new PlantaDocenteAdministrativa();
    ...
    /**
     * Updates an existing PlantaDocenteAdministrativa model.
    ...
    /**
     * Deletes an existing PlantaDocenteAdministrativa model.
    ...
    /**
     * Finds the PlantaDocenteAdministrativa model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PlantaDocenteAdministrativa the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PlantaDocenteAdministrativa::findOne($id)) !== null) {
            return $model;
        }
