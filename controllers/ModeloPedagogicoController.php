...
/**
 * ModeloPedagogicoController implements the CRUD actions for ModeloPedagogico model.
 */
class ModeloPedagogicoController extends Controller
{
    ...
    /**
     * Lists all ModeloPedagogico models.
     * @return mixed
     */
    public function actionIndex($guardado = 0)
    {
        $dataProvider = new ActiveDataProvider([
            'query' => ModeloPedagogico::find(),
        ]);
    ...
    /**
     * Displays a single ModeloPedagogico model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new ModeloPedagogico model.
    ...
    public function actionCreate()
    {
		...
		if( Yii::$app->request->post('ModeloPedagogico') )
			$data = Yii::$app->request->post('ModeloPedagogico');
		...
		for( $i = 0; $i < $count; $i++ )
		{
			$models[] = new ModeloPedagogico();
		}
							
        $dataTiposDocumento  = TiposDocumentos::find()->where( 'estado=1' )->andWhere( "categoria='Modelo Pedagogico'")->all();
		$tiposDocumento 	 = ArrayHelper::map( $dataTiposDocumento, 'id', 'descripcion' );
		
		$dataEstados  = Estados::find()->where( 'id=1' )->all();
        $estados 	  = ArrayHelper::map( $dataEstados, 'id', 'descripcion' );
        
        if (ModeloPedagogico::loadMultiple($models, Yii::$app->request->post() )) {			
			...
			foreach( $models as $key => $model) {
				...
				if( $file ){
					...
					$carpeta = "../documentos/modeloPedagogico/";
					if (!file_exists($carpeta)) {
						mkdir($carpeta, 0777, true);
					}
					
					//Construyo la ruta completa del archivo a guardar
					$rutaFisicaDirectoriaUploads  = "../documentos/modeloPedagogico/";
			...
			if (!ModeloPedagogico::validateMultiple($models)) {
    ...
    /**
     * Updates an existing ModeloPedagogico model.
    ...
    /**
     * Deletes an existing ModeloPedagogico model.
    ...
    /**
     * Finds the ModeloPedagogico model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return ModeloPedagogico the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = ModeloPedagogico::findOne($id)) !== null) {
            return $model;
        }
