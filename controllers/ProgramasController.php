...
class ProgramasController extends Controller
{
    ...
    public function actionCreate()
    {
		...
		if( Yii::$app->request->post('Programas') )
			$data = Yii::$app->request->post('Programas');
		...
		for( $i = 0; $i < $count; $i++ )
		{
			$models[] = new Programas();
		}
							
        $dataTiposDocumento  = TiposDocumentos::find()->where( 'estado=1' )->andWhere( "categoria='Programas'")->all();
		$tiposDocumento 	 = ArrayHelper::map( $dataTiposDocumento, 'id', 'descripcion' );
		
		$dataEstados  = Estados::find()->where( 'id=1' )->all();
        $estados 	  = ArrayHelper::map( $dataEstados, 'id', 'descripcion' );

        if (Programas::loadMultiple($models, Yii::$app->request->post() )) {			
			...
			foreach( $models as $key => $model) {
				...
				if( $file ){
					...
					$carpeta = "../documentos/programas/";
					if (!file_exists($carpeta)) {
						mkdir($carpeta, 0777, true);
					}
					
					//Construyo la ruta completa del archivo a guardar
					$rutaFisicaDirectoriaUploads  = "../documentos/programas/";
			...
			if (!Programas::validateMultiple($models)) {
