...
Descripción: Controlador Documentos Servicio Social
---------------------------------------
**********/
 ...
 */
class DocumentosServicioSocialController extends Controller
{
    /**
     * @inheritdoc
     */
	...
	function actionAgregarCampos(){
		...
		$dataTiposDocumento  = TiposDocumentos::find()->where( 'estado=1' )->andWhere( "categoria='Servicio Social'")->all();
    ...
    public function actionIndex( $idInstitucion = 0, $guardado = 0 )
    {
		...
		if( $idInstitucion > 0 )
		{
			...
			$dataProvider->query
				->innerJoin( "tipos_documentos td", "td.id=id_tipo_documentos" )
				->andWhere( 'documentos_oficiales.estado=1' )
				->andWhere( 'id_instituciones='.$idInstitucion )
				->andWhere( "td.categoria='Servicio Social'" );	//21 es el id de servicio social
		...
		if (DocumentosOficiales::loadMultiple($models, Yii::$app->request->post() )) {			
			...
			foreach( $models as $key => $model) {
				...
				if( $file ){
					...
					$carpeta = "../documentos/Sevicio Social/".$institucion->codigo_dane;
					if (!file_exists($carpeta)) {
						mkdir($carpeta, 0777, true);
					}
					
					//Construyo la ruta completa del archivo a guardar
					$rutaFisicaDirectoriaUploads  = "../documentos/Sevicio Social/".$institucion->codigo_dane."/";
