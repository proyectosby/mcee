...
class EstimulosController extends Controller
{
    ...
    public function actionCreate()
    {
		...
		if( Yii::$app->request->post('Estimulos') )
			$data = Yii::$app->request->post('Estimulos');
		...
		for( $i = 0; $i < $count; $i++ )
		{
			$models[] = new Estimulos();
		}
							
        $dataTiposDocumento  = TiposDocumentos::find()->where( 'estado=1' )->andWhere( "categoria='Estimulos'")->all();
		$tiposDocumento 	 = ArrayHelper::map( $dataTiposDocumento, 'id', 'descripcion' );
		
		$dataEstados  = Estados::find()->where( 'id=1' )->all();
        $estados 	  = ArrayHelper::map( $dataEstados, 'id', 'descripcion' );

        if (Estimulos::loadMultiple($models, Yii::$app->request->post() )) {			
			...
			foreach( $models as $key => $model) {
				...
				if( $file ){
					...
					$carpeta = "../documentos/estimulos/";
					if (!file_exists($carpeta)) {
						mkdir($carpeta, 0777, true);
					}
					
					//Construyo la ruta completa del archivo a guardar
					$rutaFisicaDirectoriaUploads  = "../documentos/estimulos/";
			...
			if (!Estimulos::validateMultiple($models)) {
